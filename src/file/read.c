...
ssize_t file_single_read(ifstream_t* file)
{
	...
	if (result < 0)
		log_error("read returned %zi: %s", result, strerror(errno));
	...
	{ /* This branch is impossible to reach if this thread has exclusive access to the queue, which it should */
		log_error("cannot add %zi characters to the queue, %zu / %zu bytes used", result, queue_count(queue), queue_capacity(queue));
		return -1;
	}
	return result;
}
